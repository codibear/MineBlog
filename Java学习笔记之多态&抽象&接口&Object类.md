...
区别  | 重载 |重写
---|----|----
签名|方法名相同，参数不同，返回值类型无关，|与父类有相同的方法名和参数返回值类型
目的|多态性|多态性（继承性）
访问权限|无要求|子类的权限必须大于等于父类权限
...
```
`父类的引用指向子类的对象`
**注意:**如果ab对象调用了一个B类型中独有的一个方法时，ab.show()。会产生编译错误，会提示A类中没有这个方法，此时只能强制转换
总的来说
...
并且我们每次写的类都是默认继承了Object类的，当你把自己编写的类什么也不写时，创建该类的对象然后调用（输入.）那么他会显示一定的方法，这些方法就是从Object类中继承来的
...
[public] [static] [final] 常量;  //只有这两中类型
